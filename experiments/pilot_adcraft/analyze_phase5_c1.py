"""Analyze Phase 5 C1: TCL β-annealing 2→10 vs TCL β=10 fixed (B1 baseline).

Produces:
  figures/phase5_c1/
    beta_schedule.png   — β_k trajectories for all 3 seeds (confirms annealing ran)
    csr_comparison.png  — CSR_c0 over steps: C1 (annealing) vs B1 (fixed), 3 seeds
    summary.txt         — steady-state CSR and return for both conditions

Usage::

    uv run python -m experiments.pilot_adcraft.analyze_phase5_c1
"""

from __future__ import annotations

import sys
from pathlib import Path

import matplotlib.pyplot as plt
import numpy as np
from tensorboard.backend.event_processing.event_accumulator import EventAccumulator

_REPO_ROOT = Path(__file__).resolve().parents[2]
if str(_REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(_REPO_ROOT))

# ── Config ────────────────────────────────────────────────────────────────────
C1_DIR = _REPO_ROOT / "runs" / "phase5_c1" / "tcl"
B1_DIR = _REPO_ROOT / "runs" / "phase5_b1" / "tcl"
OUT_DIR = _REPO_ROOT / "figures" / "phase5_c1"

SEEDS = [1, 2, 3]
K_COSTS = 3
STEADY_FRACTION = 0.20
SMOOTH_WINDOW = 20

# ── Helpers ───────────────────────────────────────────────────────────────────

def find_tb_run(seed_dir: Path) -> Path | None:
    tb = seed_dir / "tb"
    if not tb.exists():
        return None
    runs = sorted(tb.iterdir(), key=lambda p: p.stat().st_mtime, reverse=True)
    return runs[0] if runs else None


def load_scalars(tb_run: Path, tags: list[str]) -> dict[str, np.ndarray]:
    ea = EventAccumulator(str(tb_run), size_guidance={"scalars": 0})
    ea.Reload()
    available = set(ea.Tags().get("scalars", []))
    out: dict[str, np.ndarray] = {}
    for tag in tags:
        if tag not in available:
            continue
        events = ea.Scalars(tag)
        out[f"{tag}/steps"] = np.array([e.step for e in events], dtype=np.int64)
        out[f"{tag}/values"] = np.array([e.value for e in events], dtype=np.float64)
    return out


def smooth(x: np.ndarray, window: int) -> np.ndarray:
    if window <= 1 or x.size == 0:
        return x.astype(np.float64)
    cs = np.cumsum(np.insert(x.astype(np.float64), 0, 0.0))
    out = np.empty_like(x, dtype=np.float64)
    for i in range(x.size):
        lo = max(0, i + 1 - window)
        out[i] = (cs[i + 1] - cs[lo]) / (i + 1 - lo)
    return out


def per_episode_csr(cost_sum: np.ndarray, ep_steps: np.ndarray) -> np.ndarray:
    safe_steps = np.where(ep_steps > 0, ep_steps, 1)
    mean_cost = cost_sum / safe_steps
    return (mean_cost <= 0.0).astype(np.float64)


def load_cell(seed_dir: Path) -> dict | None:
    tb_run = find_tb_run(seed_dir)
    if tb_run is None:
        return None
    tags = [
        "rollout/episode_return",
        "rollout/episode_steps",
        *(f"rollout/episode_cost_{k}" for k in range(K_COSTS)),
        *(f"train/beta_{k}" for k in range(K_COSTS)),
    ]
    s = load_scalars(tb_run, tags)
    if "rollout/episode_steps/values" not in s:
        return None

    ep_steps = s["rollout/episode_steps/values"]
    ep_steps_axis = s["rollout/episode_steps/steps"]

    csr_per_k: dict[int, tuple[np.ndarray, np.ndarray]] = {}
    for k in range(K_COSTS):
        tag = f"rollout/episode_cost_{k}"
        if f"{tag}/values" not in s:
            continue
        cost = s[f"{tag}/values"]
        n = min(cost.shape[0], ep_steps.shape[0])
        csr = per_episode_csr(cost[:n], ep_steps[:n])
        csr_per_k[k] = (ep_steps_axis[:n], smooth(csr, SMOOTH_WINDOW))

    n_ep = len(ep_steps)
    steady_start = int(n_ep * (1.0 - STEADY_FRACTION))

    ret_vals = s.get("rollout/episode_return/values", np.array([]))
    ret_steps = s.get("rollout/episode_return/steps", np.array([]))

    betas: dict[int, tuple[np.ndarray, np.ndarray]] = {}
    for k in range(K_COSTS):
        tag = f"train/beta_{k}"
        if f"{tag}/values" in s:
            betas[k] = (s[f"{tag}/steps"], s[f"{tag}/values"])

    # Steady-state metrics
    ss_csr = {}
    for k, (steps, vals) in csr_per_k.items():
        n = len(vals)
        ss_start = int(n * (1.0 - STEADY_FRACTION))
        ss_csr[k] = float(vals[ss_start:].mean()) if ss_start < n else float("nan")

    n_ret = len(ret_vals)
    ss_ret_start = int(n_ret * (1.0 - STEADY_FRACTION))
    ss_return = float(ret_vals[ss_ret_start:].mean()) if ss_ret_start < n_ret else float("nan")

    return {
        "csr_per_k": csr_per_k,
        "betas": betas,
        "ret": (ret_steps, ret_vals),
        "ss_csr": ss_csr,
        "ss_return": ss_return,
    }


# ── Main ──────────────────────────────────────────────────────────────────────

def main() -> None:
    OUT_DIR.mkdir(parents=True, exist_ok=True)

    c1_cells: dict[int, dict] = {}
    b1_cells: dict[int, dict] = {}

    for seed in SEEDS:
        c1_cell = load_cell(C1_DIR / f"seed={seed}")
        if c1_cell:
            c1_cells[seed] = c1_cell
        b1_cell = load_cell(B1_DIR / f"seed={seed}")
        if b1_cell:
            b1_cells[seed] = b1_cell

    if not c1_cells:
        print("C1 run not found or not yet complete. Re-run after training finishes.")
        return

    # ── Figure 1: β schedule ─────────────────────────────────────────────────
    fig, ax = plt.subplots(figsize=(7, 3.5))
    for seed, cell in c1_cells.items():
        for k, (steps, vals) in cell["betas"].items():
            ax.plot(steps, vals, label=f"seed={seed} β_{k}", alpha=0.8)
    ax.set_xlabel("Training step")
    ax.set_ylabel("β")
    ax.set_title("C1 — β annealing schedule (TCL, 2→10 linear)")
    ax.legend(fontsize=7, ncol=3)
    ax.axhline(2.0, linestyle=":", color="gray", alpha=0.5, label="β_min=2")
    ax.axhline(10.0, linestyle="--", color="gray", alpha=0.5, label="β_max=10")
    ax.set_ylim(0, 12)
    fig.tight_layout()
    fig.savefig(OUT_DIR / "beta_schedule.png", dpi=150)
    plt.close(fig)
    print(f"Saved: {OUT_DIR / 'beta_schedule.png'}")

    # ── Figure 2: CSR_c0 comparison ──────────────────────────────────────────
    fig, axes = plt.subplots(1, 2, figsize=(12, 4), sharey=True)
    colors = {1: "#1f77b4", 2: "#ff7f0e", 3: "#2ca02c"}

    for ax, (cells, label) in zip(
        axes,
        [(c1_cells, "C1: β 2→10 (annealing)"), (b1_cells, "B1: β=10 fixed")],
    ):
        for seed, cell in cells.items():
            if 0 not in cell["csr_per_k"]:
                continue
            steps, vals = cell["csr_per_k"][0]
            ax.plot(steps, vals, color=colors[seed], alpha=0.8,
                    label=f"seed={seed} (ss={cell['ss_csr'].get(0, float('nan')):.3f})")
        ax.axhline(0.9, linestyle="--", color="k", alpha=0.4, linewidth=0.8)
        ax.set_title(label)
        ax.set_xlabel("Training step")
        ax.set_ylabel("CSR$_{c_0}$ (utilization)")
        ax.set_ylim(-0.05, 1.1)
        ax.legend(fontsize=8)

    fig.suptitle("TCL: β-annealing (C1) vs fixed β=10 (B1) — CSR_c0 over training",
                 fontsize=11)
    fig.tight_layout()
    fig.savefig(OUT_DIR / "csr_comparison.png", dpi=150)
    plt.close(fig)
    print(f"Saved: {OUT_DIR / 'csr_comparison.png'}")

    # ── Summary ───────────────────────────────────────────────────────────────
    lines = ["Phase 5 C1 — β-annealing 2→10 vs B1 fixed β=10\n",
             "=" * 55, ""]
    lines.append(f"{'':>20} {'CSR_c0':>8} {'CSR_c1':>8} {'CSR_c2':>8} {'Return_ss':>10}")
    lines.append("-" * 55)

    for label, cells in [("C1 (annealing)", c1_cells), ("B1 (fixed)", b1_cells)]:
        if not cells:
            lines.append(f"{label:>20}  (no data)")
            continue
        for seed, cell in sorted(cells.items()):
            csrs = " ".join(f"{cell['ss_csr'].get(k, float('nan')):8.3f}"
                            for k in range(K_COSTS))
            lines.append(f"{label:>20} seed={seed}  {csrs} {cell['ss_return']:10.1f}")
        # Mean
        c0_vals = [c["ss_csr"].get(0, float("nan")) for c in cells.values()]
        ret_vals = [c["ss_return"] for c in cells.values()]
        lines.append(
            f"{'MEAN':>20}{'':7}"
            f"{np.nanmean(c0_vals):8.3f}"
            f"{'':8}"
            f"{'':8}"
            f"{np.nanmean(ret_vals):10.1f}"
        )
        lines.append("")

    summary_path = OUT_DIR / "summary.txt"
    with open(summary_path, "w") as f:
        f.write("\n".join(lines))
    print(f"\n{'=' * 55}")
    print("\n".join(lines))
    print(f"\nSaved: {summary_path}")


if __name__ == "__main__":
    main()
